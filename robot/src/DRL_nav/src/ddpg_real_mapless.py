...
from environment_real_mapless import Env
...
STATE_DIMENSION = 14
ACTION_DIMENSION = 2
ACTION_V_MAX = 0.22 # 0.1 m/s 
ACTION_W_MAX = 2 #4 00.4 rad/s
# world = 'stage_2'
world='real'
if is_training:
...
class RunManager():
    ...
    def setCoord(self, model):
        pub_received = rospy.Publisher('received', String, queue_size=1)
        print("Received x and y: ", model.x, model.y)
        if (model.x!=0 and model.y!=0):
            pub_received.publish("received")
            print("Navigate towards x and y: ", model.x, model.y)
            globals.x_coord=model.x
            globals.y_coord=model.y
            globals.z_coord=model.z
            globals.init=globals.init+1
            pub_result = rospy.Publisher('result', Float32, queue_size=5)
            pub_done = rospy.Publisher('done', String, queue_size=1)

            result = Float32()
            env = Env(ACTION_DIMENSION,model.x,model.y)
            before_training = 4
