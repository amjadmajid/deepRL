...
class DetectorManager():
    def __init__(self):
        ...
        # Define publishers
        self.pub = rospy.Publisher('findObjects', Object, queue_size=1)
        # Spin
        rospy.spin()

    def new_imageCb(self,data):
        ...
        #path = '/home/serges/catkin_ws/test'
        ...
        for (_, _, _) in data_loader:
            for idx, (preds, _, _) in enumerate(predictor.dataset(data1)):
                ...
                if args.output_directory is None:
                    splits = os.path.split("camera_image")
                    output_path = os.path.join(splits[0], 'out_' + splits[1])
                else:
                    file_name = os.path.basename("camera_image")
                    output_path = os.path.join(
                        args.output_directory, 'out_' + file_name)
                im_name = os.path.basename("camera_image")
                rospy.loginfo(f'{idx} image {im_name} saved as {output_path}')
        ...
        json_dump=json.dumps(dic_out)
        resp = json.loads(json_dump, parse_int=float)
        #print (resp['xyz_pred'])
    ...
    def imageCb(self,data):
        ...
        resp = dic_out["xyz_pred"]
        resp_sd=dic_out["social_distance"]
        msg = Object()
        for index, row in enumerate(resp):
            #resp[index] = list(map(float, row))
            msg.x=resp[0][0]
            msg.y=resp[0][1]
            msg.z=resp[0][2]
