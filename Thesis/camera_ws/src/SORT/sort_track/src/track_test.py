...
class Person:
	...
	def breaching (self, key):
		...
		if (euc_distance<0.5 and self.id != other.id):
			print ('x,y,otherx,othery: ',x,y,x_other,y_other,euc_distance)
			self.breach=True
			other.breach=True
		...
		if (trackers.size!=0):
			...
			if ((new_ID not in id_set)):
				for key, value in person_dict.items():
					if ((value.xmin-0.1<box.xmin<value.xmin+0.1) and (value.xmax-0.1<box.xmax<value.xmax+0.1) and (value.ymin-0.1<box.ymin<value.ymin+0.1) and (value.ymax-0.1<box.ymax<value.ymax+0.1)):
						track_bool=True
						new_ID=key
				...
				if not (track_bool):
					...
					print ('x = ',new_person.x)
					print( 'y = ', new_person.y)
					x_val=new_person.x
					y_val=new_person.y
